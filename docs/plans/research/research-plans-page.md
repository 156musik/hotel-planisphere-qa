...
| 会員限定・プレミアム限定の `card-header` が表示されること | `src/plans.js:32-38` |
| 未ログイン時に「会員登録」「ログイン」がナビに表示されること | `ja/plans.html:30-37` |
| ログイン済み時に「マイページ」「ログアウト」がナビに表示されること | `src/lib/session.js:102-110` |
| おすすめプランの `card-footer` に「本日限り」が表示されること | `ja/plans.html:68-70` |
| SP幅で `h1` が非表示になること | `ja/plans.html:16`（`d-none d-lg-block`） |
